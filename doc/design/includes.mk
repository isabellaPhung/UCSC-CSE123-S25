INSERT_MD += ../common/ethics_statement.md

INSERT_MD += ../common/existing_designs.md
INSERT_MD += ../common/sustainability_statement.md

INSERT_MD += ../common/tests/test_batterylife.md
INSERT_MD += ../common/tests/test_batterylifecycle.md
INSERT_MD += ../common/tests/test_bootTime.md
INSERT_MD += ../common/tests/test_bootTime_report.md
INSERT_MD += ../common/tests/test_chargespeed.md
INSERT_MD += ../common/tests/test_chemical.md
INSERT_MD += ../common/tests/test_cloud.md
INSERT_MD += ../common/tests/test_cloud_report.md
INSERT_MD += ../common/tests/test_emi.md
INSERT_MD += ../common/tests/test_factoryReset.md
INSERT_MD += ../common/tests/test_lcd.md
INSERT_MD += ../common/tests/test_manyToOne.md
INSERT_MD += ../common/tests/test_manyToOne_report.md
INSERT_MD += ../common/tests/test_offline.md
INSERT_MD += ../common/tests/test_offline_report.md
INSERT_MD += ../common/tests/test_persistence.md
INSERT_MD += ../common/tests/test_persistence_report.md
INSERT_MD += ../common/tests/test_portDurability.md
INSERT_MD += ../common/tests/test_restoration.md
INSERT_MD += ../common/tests/test_restoration_report.md
INSERT_MD += ../common/tests/test_security.md
INSERT_MD += ../common/tests/test_security_report.md
INSERT_MD += ../common/tests/test_softwareUpdate.md
INSERT_MD += ../common/tests/test_soundhaptics.md
INSERT_MD += ../common/tests/test_sync_basic.md
INSERT_MD += ../common/tests/test_sync_basic_report.md
INSERT_MD += ../common/tests/test_sync_dual.md
INSERT_MD += ../common/tests/test_sync_dual_report.md
INSERT_MD += ../common/tests/test_sync_redo.md
INSERT_MD += ../common/tests/test_sync_redo_report.md
INSERT_MD += ../common/tests/test_syncTime.md
INSERT_MD += ../common/tests/test_syncTime_report.md
INSERT_MD += ../common/tests/test_tensile.md
INSERT_MD += ../common/tests/test_thermal.md
INSERT_MD += ../common/tests/test_thermalShock.md
INSERT_MD += ../common/tests/test_UV.md
INSERT_MD += ../common/tests/test_wifi.md
INSERT_MD += ../common/tests/test_wifi_report.md

INSERT_MD += ../common/personas/camillia.md
INSERT_MD += ../common/personas/elijah.md
INSERT_MD += ../common/personas/linda.md
INSERT_MD += ../common/personas/mary.md
INSERT_MD += ../common/personas/peyton.md

INSERT_MD += ../common/need_statement.md
INSERT_MD += ../common/goal_statement.md

INSERT_MD += ../common/design_objective.md
INSERT_MD += ../common/design_objective_table.md

INSERT_MD += ../common/morphological_chart.md
INSERT_MD += ../common/brainstorm.md

INSERT_MD += ../common/decision_table.md

INSERT_MD += ../common/design_for_manufacture_and_assembly.md
INSERT_MD += ../common/lifecycle_assessment.md

INSERT_MD += ../common/executive_summary.md

INSERT_MD += ../common/entry_design.md
INSERT_MD += ../common/device_statemachine.md

INSERT_MD += ../common/test_plan.md

INSERT_MD += planning.md

INSERT_MD += ../common/third-party_integration.md
INSERT_MD += ../common/manufactured_design.md
INSERT_MD += ../common/device_ui_requirements.md
INSERT_MD += ../common/configurable_settings.md
INSERT_MD += ../common/webapp_design_spec.md
INSERT_MD += ../common/prototype_vs_manufactured.md
INSERT_MD += ../common/test_suite.md
INSERT_MD += ../common/Finalized_Specs.md
INSERT_MD += ../common/why.md

INSERT_MD += ../common/startManual/startManual.md

INSERT_MD += server_design.md
INSERT_MD += hardware_design.md
INSERT_MD += server_communication.md
INSERT_MD += prototype_hardware.md
INSERT_MD += prototype_software.md
INSERT_MD += prototype_server.md

INSERT_MD += phone_app_design.md

INSERT_MD += eval_hardware.md

INSERT_MD += akanksha_review.md 
INSERT_MD += isabella_review.md 
INSERT_MD += lennan_review.md 
INSERT_MD += mason_review.md
INSERT_MD += sulaiman_review.md

INSERT_TEX := ../common/UI_user_tutorial/user_tutorial.tex
INSERT_TEX += prototype_tests.tex
INSERT_TEX += prototype_test_results.tex
INSERT_TEX += manufactured_tests.tex
INSERT_TEX += ../common/tests/test_drop.tex
INSERT_TEX += ../common/UI_user_tutorial/user_tutorial.tex
INSERT_TEX += wifi_provisioning.tex
