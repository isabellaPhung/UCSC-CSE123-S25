...
/// @param datetim Datetime in ISO 8601 format
/// @param priority The importance of the task (0-9)
/// @param completed    Whether the task is complete	// TODO: Convert to flag system for additional data
...
int PrintEntry(sqlite3 *db, sqlite3_int64 id)
{
    ...
    while (sqlite3_step(stmt) != SQLITE_DONE) {
        ...
        printf("ID: %lld | Name: %s | Due: %s | Priority: %d | Completed: %d | Description: %s\n",
               id, name, datetime, priority, completed, description ? description : "N/A");
    }
